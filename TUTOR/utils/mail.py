...
def send_user_change_password_email(email, user_serialized_id):
	...
	msg = MailMessage(string, sender=current_app.config.get("MAIL_USERNAME"), 
	recipients=[email])
	msg.body = f'''This is a confirmation that the password for your SPORT account has just been changed.
...
def send_email_change_request_email(email, serialized_id_and_email):
	#send mail
	string = """SPORT change email"""
	msg = MailMessage(string, sender=current_app.config.get("MAIL_USERNAME"), 
	recipients=[email])
	msg.body = f'''you requested to change your SPORT account email to this email, please click the following link to 
	finish the change: 
...
def send_deny_email_change_email(email, user_serialized_id_and_email):
	#send mail
	string = """SPORT deny email change"""
	msg = MailMessage(string, sender=current_app.config.get("MAIL_USERNAME"), 
	recipients=[email])
	msg.body = f'''someone requetsed to change your SPORT account email, if that wasn't you please click the following link to 
	revert the change
